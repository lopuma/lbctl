...
class DAO():
    ...
    def connect_database(self):
        if self.conexion.is_connected():
            success(
                f"[ 1 ] The DataBase is connected on host {config('MYSQL_HOST')} the PORT: {config('MYSQL_PORT')}")
        else:
            raise MySQLConnectionError(
                f"[ 1 ] No se pudo conectar al servidor MySQL al host : {config('MYSQL_HOST')} y puerto {config('MYSQL_PORT')} especificados. Por favor, revise el hosts o la dirección IP y el puerto.")
    ...
    def insert_databook(self, book_data):
        if self.conexion.is_connected():
            try:
                ...
                value = (book_data["title"], book_data["author"], book_data["editorial"], book_data["isbn"], book_data["category"],
                         book_data["language"], book_data["collection"], book_data["purchase_date"], book_data["observation"], book_data["numReference"])
                cursor.execute(query, value)
                self.conexion.commit()
    ...
    def update_databook(self, idBook, book_data):
        ...
        if self.conexion.is_connected():
            try:
                ...
                value = (dataBook["title"], dataBook["author"], dataBook["editorial"], dataBook["isbn"], dataBook["category"], dataBook["language"],
                         dataBook["collection"], dataBook["purchase_date"], dataBook["observation"], dataBook["numReference"], bookID)
                cursor.execute(query, value)
                self.conexion.commit()
            except MySQLConnectionError as e:
                error("Error de integridad:", e)
    ...
    def update_nameCover(self, name_cover, idBook):
        ...
        try:
            ...
            if result:
                ...
                query = (
                    "INSERT INTO coverBooks (bookID, nameCover) VALUES (%s, %s)")
